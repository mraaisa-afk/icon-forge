...
**F5 Grid drift.** Projection profiles of ink row/column sums → empty-column valleys → if regularity (1 − MAD/median) > 0.75, treat it as a grid. Snap a component only if its bbox fits inside one cell inflated by −2px; otherwise flag `SpansMultipleCells` and send it back to watershed. **Grid detection is a hint layer, never authoritative** — this is what lets one algorithm handle both neat sprite sheets and scattered layouts.
